...
OUTPUTS (stored as attributes of the find_galaxy class):
      ...
      theta = Position angle measured clock-wise from the image X axis,
          IMPORTANT: The standard astronomical PA is measured counter-clockwise
              from the image Y-axis (assumed to coincide with North).
              This implies that the astronomical PA = 270 - theta
...
class find_galaxy(object):
    ...
    def __init__(self, img, fraction=0.1, plot=False, quiet=False,
        ...
        if not quiet:
            ...
            print(' Astro PA (deg): %.1f' % np.mod(270 - self.theta, 180))
