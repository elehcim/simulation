...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--conc', dest='c', default=0.0,
                        help='NFW halo concentration factor. Use 0 to compute c automatically using Wechsler2002 or Strigari2007 models',
                        type=float)
    ...
    parser.add_argument('--plot_integrand', help='Plot the function to integrate to get the radial period',
                        action="store_true")
    ...
    rp = args.rp * kpc_in_km  # * u.kpc # 15 # kpc. Pericenter distance
    ra = args.ra * kpc_in_km  # * u.kpc
    r = args.r   * kpc_in_km  # * u.kpc
    ...
    nfw = NFWPotential(M_h, args.c)
    print("Halo parameters:")
    print("  Mass                  M_h = {:.1e} Msol".format(M_h))
    print("  Concentration factor    c =", nfw.c)
    print("  Halo scaled density rho_s = {:.2e} kg/m^3 ({:.2e} Msol/km^3) ({:.2e} 10^10 Msol/kpc^3)".format(
        nfw.rho_s * Msol / 1e9, nfw.rho_s, nfw.rho_s * kpc_in_km ** 3 / 1e10))
    print("  Halo scaled radius    R_s = {:.2f} kpc".format(nfw.R_s / kpc_in_km))
    ...
    if args.output_file is None:
        gic_file = "{}.kicked_p{}_a{}_r{}_c{:.2f}.gic".format(os.path.basename(args.input_file), *(apsis + [nfw.c]))
        if args.prefix:
